...
| `tests/run_tests.m` puts `tests/stub` on the path once, before the first call loads the MEX file, and `tests/test_helpers.m` changes nothing about the path. The stub is removed again only on a machine that has Psychtoolbox, just before `tests/gl`. | `test_helpers` first added and removed the stub directory itself, with an `onCleanup`. The `build . linux . Octave 10.1.0` CI job then died with a segmentation fault at the moment `test_helpers` started, while Octave 6.4 in the same image, Octave 10.1 on Windows, and MATLAB all passed. The two sibling projects crashed the same way with the same pattern, so the trigger is rewriting the load path while a MEX file is loaded, not anything in this binding. Doing it once, before the MEX loads, removes the trigger from the platform that failed. `PsychNanoVGSetup` is idempotent for the same reason: `PsychNanoVGOpen` calls it on every open, and it used to rewrite the load path each time. |
